...
class FinologBiz(BaseFinologBiz):
    ...
    2. Current month - current month income with progress bar;
    3. Current month goal - current month goal income with progress bar;
    4. Income goal - goal frame with income's sum on current year;
    5. Income chart - chart frame with income by month.
    """
    ...
    def get_lametric_frames(self):
        return self.get_summary_frame(), self.get_current_month_transaction_sum(), \
               self.get_current_month_goal_income(), self.get_income_goal_frame(), self.get_income_chart_frame(),
    ...
    def _get_current_month_transactions_sum(self) -> float:  # in thousands
        ...
        for transaction in self.get_transactions_response(**get_params).json():
            value, report_date = transaction['value'], datetime.strptime(transaction['report_date'], '%Y-%m-%d %H:%M:%S')
